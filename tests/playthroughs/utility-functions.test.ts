...
describe('Playthroughs Store - Utility Functions', () => {
  ...
  describe('getLocationFromComboboxId', () => {
    ...
    it('should extract location from body combobox id', () => {
      const result = playthroughActions.getLocationFromComboboxId('route-1-body');
      expect(result).toEqual({ locationId: 'route-1', field: 'body' });
    });

    it('should handle complex location ids with head suffix', () => {
      const result = playthroughActions.getLocationFromComboboxId('viridian-city-gym-head');
      expect(result).toEqual({ locationId: 'viridian-city-gym', field: 'head' });
    });

    it('should handle complex location ids with body suffix', () => {
      const result = playthroughActions.getLocationFromComboboxId('viridian-city-gym-body');
      expect(result).toEqual({ locationId: 'viridian-city-gym', field: 'body' });
    });

    it('should return original id when no suffix present', () => {
      const result = playthroughActions.getLocationFromComboboxId('some-complex-location-name');
      expect(result).toEqual({ locationId: 'some-complex-location-name', field: 'head' });
    });
    ...
    it('should handle edge case where location name contains head or body', () => {
      ...
      const result = playthroughActions.getLocationFromComboboxId('headbutt-tree-1');
    ...
    it('should handle single suffix correctly', () => {
      const result = playthroughActions.getLocationFromComboboxId('route-1-single');
      expect(result).toEqual({ locationId: 'route-1', field: 'head' });
    });
  });
