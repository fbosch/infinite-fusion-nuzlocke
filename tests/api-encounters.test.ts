...
describe('Encounters API', () => {
  it('should include egg locations with -1 ID', async () => {
    ...
    const request = new NextRequest('http://localhost:3000/api/encounters?gameMode=classic');
    ...
    const nonEggRoutes = data.filter((route: any) => !eggRouteNames.includes(route.routeName));
  ...
  it('should work for both classic and remix modes', async () => {
    // Test classic mode
    const classicRequest = new NextRequest('http://localhost:3000/api/encounters?gameMode=classic');
    ...
    // Test remix mode
    const remixRequest = new NextRequest('http://localhost:3000/api/encounters?gameMode=remix');
