...
export type SpecialLocationType = typeof SPECIAL_LOCATIONS[keyof typeof SPECIAL_LOCATIONS];

/**
 * Check if a location ID is a special location
 */
export function isSpecialLocation(locationId: string): boolean {
  return Object.values(SPECIAL_LOCATIONS).includes(locationId as SpecialLocationType);
}
 ...
 */
export function getSpecialLocationEncounterType(locationId: string): 'starter' | 'gift' | 'trade' | null {
