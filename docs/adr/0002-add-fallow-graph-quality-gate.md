...
Wire the gate into the existing code-quality path rather than replacing TypeScript, Vitest, or Biome. CI runs `pnpm run validate` first and then `pnpm run quality:graph`. Worktrunk pre-merge hooks run the same graph-quality audit after type-checking and tests.

Install Fallow's agent guidance so local agents run `fallow audit --format json --quiet --explain` before commit or push and treat only `fail` verdicts as blockers.
